<template>
  <div class="min-h-screen bg-[#eef0f7] flex flex-col pb-28 font-nunito">
    ...
    <div v-else class="px-4 flex flex-col gap-3">
      ...
      <div class="flex items-end justify-center gap-3 py-6">
        ...
        <div class="flex flex-col items-center gap-2">
          <div class="relative">
            ...
            <span
              class="absolute -bottom-1 -right-1 bg-gray-400 text-white text-xs font-black w-6 h-6 rounded-full flex items-center justify-center shadow">2</span>
          </div>
          <p class="text-xs font-bold text-gray-500 max-w-[72px] truncate text-center">
            {{ users[1]?.profiles?.full_name?.split(" ")[0] || "—" }}
          </p>
          <div class="bg-white rounded-xl px-3 py-1 shadow-sm">
            <span class="text-gray-500 text-xs font-black">{{ users[1]?.coins || 0 }} 🪙</span>
          </div>
        ...
        <div class="flex flex-col items-center gap-2 -mb-2">
          ...
          <div class="relative">
            ...
            <span
              class="absolute -bottom-1 -right-1 bg-cyan-400 text-white text-xs font-black w-6 h-6 rounded-full flex items-center justify-center shadow">1</span>
          </div>
          ...
          <div class="bg-cyan-50 rounded-xl px-3 py-1 shadow-sm">
            <span class="text-cyan-500 text-xs font-black">{{ users[0]?.coins || 0 }} 🪙</span>
          </div>
          <div
        ...
        <div class="flex flex-col items-center gap-2">
          <div class="relative">
            ...
            <span
              class="absolute -bottom-1 -right-1 bg-orange-400 text-white text-xs font-black w-6 h-6 rounded-full flex items-center justify-center shadow">3</span>
          </div>
          <p class="text-xs font-bold text-gray-500 max-w-[72px] truncate text-center">
            {{ users[2]?.profiles?.full_name?.split(" ")[0] || "—" }}
          </p>
          <div class="bg-orange-50 rounded-xl px-3 py-1 shadow-sm">
            <span class="text-orange-400 text-xs font-black">{{ users[2]?.coins || 0 }} 🪙</span>
          </div>
      ...
      <div v-for="(user, index) in users.slice(3)" :key="user.user_id"
        ...
        <div
          ...
          <img v-if="user.profiles?.avatar_url" :src="user.profiles.avatar_url" class="w-full h-full object-cover" />
        ...
        <div class="bg-purple-50 rounded-xl px-3 py-1">
          <span class="text-purple-500 text-xs font-black">{{ user.coins }} 🪙</span>
        </div>
      </div>

      <!-- Empty state -->
      <div v-if="users.length === 0" class="flex flex-col items-center justify-center py-20 gap-3">
