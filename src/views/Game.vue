<template>
  <div class="min-h-screen bg-gradient-to-b from-[#FFF7F0] to-[#F7F9FC] px-4 pt-6 pb-28">
    <div class="max-w-lg mx-auto">
      ...
      <div v-if="screen === 'setup'" class="space-y-4">
        ...
        <div class="bg-white rounded-3xl border border-slate-200 p-2 shadow-sm">
          <div class="grid grid-cols-2 sm:grid-cols-4 gap-1.5">
            <button v-for="m in modes" :key="m.id" @click="gameMode = m.id" :class="gameMode === m.id
              ? 'bg-gradient-to-br ' + m.grad + ' text-white shadow-md scale-[1.02]'
              ...
              <span class="text-[11px] font-bold leading-tight text-center">{{ m.label }}</span>
            </button>
          </div>
          <p class="text-xs text-slate-500 text-center mt-2 px-2 transition-all">{{ currentMode.hint }}</p>
        </div>

        <!-- Word list builder (memory / quiz / type modes) -->
        <div v-if="gameMode !== 'battle'" class="bg-white rounded-3xl border border-slate-200 p-5 shadow-sm">
          ...
          <div class="mb-4">
            <p class="text-sm font-semibold text-slate-700 mb-2 flex items-center gap-1">
          ...
          <button @click="startGame" :disabled="validPairs.length < currentMode.minPairs"
          ...
          <p v-if="validPairs.length < currentMode.minPairs" class="text-xs text-red-400 text-center mt-2">
            Kamida {{ currentMode.minPairs }} ta to'liq juft kerak
          </p>
        </div>

        <!-- Team Battle config -->
        <div v-else class="bg-white rounded-3xl border border-slate-200 p-5 shadow-sm space-y-4">
          <div>
            <h2 class="font-black text-lg text-slate-900 mb-1 flex items-center gap-1.5">
              <Users :size="18" class="text-red-500" /> Jamoa jangi
            </h2>
            <p class="text-sm text-slate-500">Mavzuni kiriting, ikki jamoa tuzing va savol-bomba kartalar
              bilan
              bellashing</p>
          </div>

          <div>
            <label class="text-xs font-bold text-slate-500 mb-1.5 block">Mavzu</label>
            <input v-model="battleTopic" placeholder="Masalan: Tarix, Geografiya, Fan..."
          ...
          <div class="grid grid-cols-2 gap-2">
            <button @click="battleConnMode = 'offline'" :class="battleConnMode === 'offline'
            ...
            <button @click="battleConnMode = 'online'" :class="battleConnMode === 'online'
              ? 'border-red-400 bg-red-50 text-red-600'
              : 'border-slate-200 text-slate-500'"
              class="py-3 rounded-2xl border-2 text-xs sm:text-sm font-bold flex flex-col items-center gap-1 transition-all">
          ...
          <div v-if="battleConnMode === 'offline'" class="space-y-3">
            <div>
              <label class="text-xs font-bold text-slate-500 mb-1.5 block">Jami o'yinchilar soni</label>
              <input v-model.number="battlePlayerCount" type="number" min="3"
                class="w-full px-3 py-2.5 rounded-xl border border-slate-200 bg-slate-50 text-sm text-slate-800 focus:outline-none focus:border-red-400 focus:ring-2 focus:ring-red-100 transition" />
            </div>
            <p class="text-xs text-slate-400 bg-slate-50 rounded-xl px-3 py-2 leading-relaxed">
              Taqsimot: <b class="text-slate-600">1 boshlovchi</b> + <b class="text-blue-600">{{ battleTeamSize }}
                (1-guruh)</b> + <b class="text-rose-600">{{ battleTeamSize }} (2-guruh)</b>
              <span v-if="battleUsedPlayers < battlePlayerCount"> — jami {{ battleUsedPlayers }} kishi
                ishtirok etadi
                (guruhlar teng bo'lishi uchun)</span>
            </p>
            <button @click="startBattleOffline"
              :disabled="battleGenerating || !battleTopic.trim() || battleTeamSize < 1"
          ...
          <div v-else class="space-y-3">
            <button @click="createBattleRoom" :disabled="!battleTopic.trim() || battleGenerating"
              class="w-full py-3.5 bg-gradient-to-r from-red-500 to-rose-600 text-white rounded-2xl font-black text-sm flex items-center justify-center gap-2 disabled:opacity-50 active:scale-95 transition">
              <Hash :size="16" /> Xona yaratish (boshlovchi bo'lish)
            </button>
            <div class="flex items-center gap-2 text-xs text-slate-400">
              <div class="flex-1 h-px bg-slate-200"></div> yoki <div class="flex-1 h-px bg-slate-200">
              </div>
            </div>
            <input v-model="battleJoinName" placeholder="Ismingiz"
            ...
            <input v-model="battleJoinCode" placeholder="Xona kodi (masalan: A1B2C)"
            ...
            <button @click="joinBattleRoom" :disabled="!battleJoinCode.trim() || !battleJoinName.trim()"
          ...
          <p v-if="battleError" class="text-xs text-red-400 flex items-center gap-1">
      ...
      <div v-else-if="screen === 'memory'">
        <StatBar :left="{ label: 'Juftlar', value: `${matchedCount}/${validPairs.length}`, color: 'text-green-500' }"
          :mid="{ label: 'Urinish', value: attempts, color: 'text-orange-500' }"
          :right="{ label: 'Vaqt', value: formatTime(elapsed), color: 'text-indigo-500' }" @reset="resetGame"
          @exit="screen = 'setup'" />

        <div class="grid grid-cols-4 gap-2">
          <div v-for="card in cards" :key="card.id" @click="flipCard(card)" class="aspect-square [perspective:600px]">
            <div class="relative w-full h-full transition-transform duration-500 [transform-style:preserve-3d]"
              ...
              <div
                ...
                <component :is="card.type === 'word' ? BookOpen : Globe" :size="22" class="text-slate-300" />
              </div>
              <!-- front (text side) -->
              <div
                ...
                :class="card.matched ? 'bg-green-50 border-green-300' : 'bg-orange-500 border-orange-400'">
                <Check v-if="card.matched" :size="14" class="absolute top-1 right-1 text-green-500" />
                <span :class="card.matched ? 'text-green-700' : 'text-white'"
                  class="text-[10px] sm:text-xs font-bold leading-tight break-words">{{ card.text }}</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <div v-else-if="screen === 'quiz'">
        <StatBar :left="{ label: 'Savol', value: `${quizIndex + 1}/${quizQueue.length}`, color: 'text-indigo-500' }"
          :mid="{ label: 'Streak', value: quizStreak, color: 'text-orange-500' }"
          :right="{ label: 'To\'g\'ri', value: quizScore, color: 'text-green-500' }" @reset="resetGame"
          @exit="screen = 'setup'" />

        <div class="w-full h-1.5 bg-slate-200 rounded-full overflow-hidden mb-5">
          <div class="h-full bg-gradient-to-r from-indigo-400 to-indigo-500 transition-all duration-500 rounded-full"
            :style="{ width: `${((quizIndex) / quizQueue.length) * 100}%` }" />
        </div>

        <div class="bg-white rounded-3xl border border-slate-200 p-6 shadow-sm text-center mb-4"
          :class="{ 'animate-pop': quizPopKey }" :key="quizPopKey">
          <p class="text-xs text-slate-400 font-semibold mb-2 flex items-center justify-center gap-1">
            <BookOpen :size="12" /> Bu so'zning tarjimasi?
          </p>
          <p class="text-2xl font-black text-slate-900">{{ quizCurrent?.word }}</p>
        </div>

        <div class="grid grid-cols-2 gap-2.5">
          <button v-for="(opt, i) in quizOptions" :key="quizPopKey + '-' + i" @click="answerQuiz(opt)"
      ...
      <div v-else-if="screen === 'type'">
        <StatBar :left="{ label: 'So\'z', value: `${typeIndex + 1}/${typeQueue.length}`, color: 'text-indigo-500' }"
          :mid="{ label: 'Xato', value: typeMistakes, color: 'text-red-400' }"
          :right="{ label: 'Vaqt', value: formatTime(typeElapsed), color: 'text-orange-500' }" @reset="resetGame"
          @exit="screen = 'setup'" />

        <div class="bg-white rounded-3xl border border-slate-200 p-8 shadow-sm text-center mb-5">
          <p class="text-xs text-slate-400 font-semibold mb-3 flex items-center justify-center gap-1">
            <PenLine :size="12" /> Tarjimasini yozing
          </p>
          <p class="text-3xl font-black text-slate-900">{{ typeCurrent?.word }}</p>
        </div>

        <form @submit.prevent="submitType" class="space-y-3">
          <input ref="typeInput" v-model="typeAnswer" autocomplete="off"
            ...
            :class="typeInputClass" placeholder="Javobingiz..." />
          <div class="flex gap-2">
            <button type="button" @click="revealType"
      ...
      <div v-else-if="screen === 'battle'">
        ...
        <div class="flex items-center justify-between mb-4 bg-white rounded-2xl border border-slate-200 px-4 py-3">
          <button @click="exitBattle"
            class="w-7 h-7 rounded-lg bg-slate-50 text-slate-400 hover:bg-slate-100 transition text-xs flex items-center justify-center">←</button>
          <span class="font-black text-sm text-slate-700 flex items-center gap-1.5">
        ...
        <div v-if="battleScreen === 'lobby'" class="animate-rise">
          <div class="bg-white rounded-3xl border border-slate-200 p-6 text-center mb-4 shadow-sm">
            ...
            <p class="text-xs text-slate-400 mt-2">Do'stlaringizga shu kodni yuboring, ular "Xonaga
              qo'shilish" orqali
              kirishadi</p>
          </div>
          ...
          <div class="grid grid-cols-2 gap-3 mb-4">
            <div class="bg-blue-50 rounded-2xl p-4 border border-blue-100">
              <p class="text-xs font-black text-blue-600 mb-2 flex items-center gap-1">
                <Users :size="13" /> 1-guruh ({{ team1Players.length }})
              </p>
              <p v-for="p in team1Players" :key="p.id" class="text-xs text-slate-600 py-0.5">{{ p.name }}
              </p>
              <p v-if="!team1Players.length" class="text-xs text-slate-300">Hali hech kim yo'q</p>
            </div>
            <div class="bg-rose-50 rounded-2xl p-4 border border-rose-100">
              <p class="text-xs font-black text-rose-600 mb-2 flex items-center gap-1">
                <Users :size="13" /> 2-guruh ({{ team2Players.length }})
              </p>
              <p v-for="p in team2Players" :key="p.id" class="text-xs text-slate-600 py-0.5">{{ p.name }}
              </p>
              <p v-if="!team2Players.length" class="text-xs text-slate-300">Hali hech kim yo'q</p>
            </div>
          </div>

          <button v-if="battleRole === 'host'" @click="startBattleOnline"
            ...
            <Loader v-if="battleGenerating" :size="16" class="animate-spin" />
            <template v-else>
              <Users :size="16" /> Jangni boshlash ({{ battleLobbyPlayers.length }} o'yinchi)
            </template>
          </button>
          <p v-else class="text-center text-sm text-slate-400 flex items-center justify-center gap-1.5">
            <Loader :size="14" class="animate-spin" /> Boshlovchi jangni boshlashini kutmoqda...
          </p>
          <p v-if="battleError" class="text-xs text-red-400 text-center mt-2">{{ battleError }}</p>
        </div>

        <!-- PLAY -->
        <div v-else-if="battleScreen === 'play'">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div v-for="team in (['team1', 'team2'] as const)" :key="team"
              class="relative overflow-hidden rounded-[2rem] border-2 p-5 shadow-lg transition-all"
              :class="[team === 'team1' ? 'border-blue-200 bg-gradient-to-br from-blue-50 via-white to-blue-100/70 shadow-blue-100' : 'border-rose-200 bg-gradient-to-br from-rose-50 via-white to-rose-100/70 shadow-rose-100', bombFlashTeam === team ? 'animate-shake' : '']">
              <div class="absolute -right-8 -top-10 h-28 w-28 rounded-full opacity-40" :class="team === 'team1' ? 'bg-blue-200' : 'bg-rose-200'"></div>
              <div class="relative flex items-start justify-between mb-4">
                <div>
                  <span class="font-black text-lg flex items-center gap-2" :class="team === 'team1' ? 'text-blue-800' : 'text-rose-800'">
                    <span class="flex h-9 w-9 items-center justify-center rounded-2xl text-white shadow-sm" :class="team === 'team1' ? 'bg-blue-500' : 'bg-rose-500'"><Users :size="18" /></span>
                    {{ team === 'team1' ? '1-guruh' : '2-guruh' }}
                  </span>
                  <p class="mt-1 text-xs font-semibold text-slate-500">{{ teamMemberCount(team) }} a'zo • {{ battleTeams[team].filter(card => !card.opened).length }} karta qoldi</p>
                </div>
                <div class="rounded-2xl bg-white/90 px-3 py-2 text-right shadow-sm ring-1 ring-black/5"><p class="text-[10px] font-bold uppercase tracking-wider text-slate-400">Hisob</p><span class="text-3xl font-black leading-none tabular-nums" :class="team === 'team1' ? 'text-blue-600' : 'text-rose-600'">{{ battleScores[team] }}</span></div>
              </div>
              <div v-if="battleActiveCard?.team === team && battleConnMode === 'online'" class="relative mb-4 rounded-2xl bg-white/80 px-3 py-2 ring-1 ring-black/5"><div class="flex items-center justify-between text-xs font-bold text-slate-600"><span>Ovozlar qabul qilinmoqda</span><span :class="team === 'team1' ? 'text-blue-600' : 'text-rose-600'">{{ activeVoteCount }}/{{ activeTeamMemberCount }}</span></div><div class="mt-2 h-1.5 overflow-hidden rounded-full bg-slate-200"><div class="h-full rounded-full transition-all duration-300" :class="team === 'team1' ? 'bg-blue-500' : 'bg-rose-500'" :style="{ width: `${activeVoteProgress}%` }"></div></div></div>
              <div class="relative grid grid-cols-5 gap-2">
                <button v-for="(card, idx) in battleTeams[team]" :key="card.id" @click="openBattleCard(team, idx)"
                  :disabled="card.opened || !!battleActiveCard || !canOpenBattleTeam(team)"
                  ...
                  <component :is="card.opened ? (card.kind === 'bomb' ? Bomb : Check) : Hash" :size="18" />
                  <span v-if="!card.opened" class="text-[10px] opacity-80">{{ idx + 1 }}</span>
                </button>
              </div>
            </div>
          </div>
          ...
          <Transition name="modal">
            <div v-if="battleActiveCard"
              ...
              <div class="bg-white rounded-[2rem] p-5 sm:p-6 max-w-md w-full shadow-2xl animate-pop">
                <div class="flex items-center justify-between gap-3 mb-4">
                  <span class="text-xs font-black px-2.5 py-1 rounded-lg"
                    :class="battleActiveCard.team === 'team1' ? 'bg-blue-100 text-blue-600' : 'bg-rose-100 text-rose-600'">
                    {{ battleActiveCard.team === 'team1' ? '1-guruh' : '2-guruh' }} savoli
                  </span>
                  <div class="text-right"><span class="flex items-center justify-end gap-1 text-xs font-black text-slate-600"><Clock :size="14" class="text-orange-500" /> {{ battleTimer }}s</span><span class="text-[10px] font-semibold text-slate-400">qolgan vaqt</span></div>
                </div>
                <div v-if="battleConnMode === 'online'" class="mb-4 rounded-2xl border border-orange-100 bg-orange-50 p-3"><div class="flex items-center justify-between gap-3"><div><p class="text-sm font-black text-slate-800">Jamoaviy ovoz</p><p class="mt-0.5 text-[11px] leading-snug text-slate-500">Har bir a'zo faqat bir marta ovoz beradi. Barcha ovoz kelganda savol yakunlanadi.</p></div><span class="shrink-0 rounded-xl bg-white px-2.5 py-2 text-sm font-black text-orange-600 shadow-sm">{{ activeVoteCount }}/{{ activeTeamMemberCount }}</span></div><div class="mt-2 h-2 overflow-hidden rounded-full bg-orange-100"><div class="h-full rounded-full bg-orange-500 transition-all duration-300" :style="{ width: `${activeVoteProgress}%` }"></div></div></div>
                ...
                <button v-if="canManuallyResolve" @click="resolveBattleCard"
        ...
        <!-- OVER -->
        <div v-else-if="battleScreen === 'over'" class="text-center py-6 animate-pop">
          <div
          ...
          <h2 class="text-2xl font-black text-slate-900">{{ battleWinnerText }}</h2>
          <div class="flex items-center justify-center gap-8 my-5">
            <div>
              <p class="text-3xl font-black text-blue-600">{{ battleScores.team1 }}</p>
              <p class="text-xs text-slate-400 mt-1">1-guruh</p>
            </div>
            <div class="text-slate-200 text-2xl font-black">:</div>
            <div>
              <p class="text-3xl font-black text-rose-600">{{ battleScores.team2 }}</p>
              <p class="text-xs text-slate-400 mt-1">2-guruh</p>
            </div>
          </div>
          <p v-if="battleRole === 'host' || battleConnMode === 'offline'"
      ...
      <Transition name="fade">
        <div v-if="gameWon"
          ...
          <span v-for="n in 16" :key="n" class="confetti absolute top-0" :style="confettiStyle(n)">
            <component :is="[Star, Sparkles, Award][n % 3]" :size="16" class="text-orange-300" />
          </span>
          <div class="bg-white rounded-3xl p-8 text-center shadow-2xl max-w-sm w-full relative animate-pop">
            <div class="flex justify-center mb-4">
              <div class="w-16 h-16 rounded-2xl bg-yellow-50 flex items-center justify-center animate-bounce-slow">
                <Trophy :size="34" class="text-yellow-500" />
              </div>
            </div>
            <h2 class="text-2xl font-black text-slate-900">Zo'r!</h2>
            <p class="text-slate-500 mt-2 text-sm">{{ winSummary }}</p>
            <div class="flex items-center justify-center gap-1.5 mt-3 text-orange-500 font-bold text-sm">
    ...
    <OnboardingTooltip pageId="Game" title="So'z O'yini"
      description="O'yin turini tanlang, kartalarni oching yoki savollarga javob bering. AI yordamida yangi so'zlar yarating" />
  </div>
</template>

<script setup lang="ts">
import { ref, computed, nextTick, onUnmounted, h, defineComponent, type PropType } from "vue";
import { useCoinStore } from "../stores/CoinStore";
import supabase from "../supabase";
import { askAIJson } from "../lib/ai";
import { saveNotification } from '../lib/Notification';
import OnboardingTooltip from '../components/OnboardingTooltip.vue';
import { Dices, Bot, Loader, RotateCcw, BookOpen, Globe, Trophy, Sparkles, Zap, PenLine, Grid3x3, Check, X, Star, Award, Flame, Eye, Send, Users, Bomb, Wifi, WifiOff, Hash, Copy, LogIn, Clock } from '@lucide/vue';
...
const StatBar = defineComponent({
  ...
  emits: ["reset", "exit"],
  setup(props, { emit }) {
    return () => h('div', { class: 'flex items-center justify-between mb-4 bg-white rounded-2xl border border-slate-200 px-4 py-3' }, [
      h('button', { onClick: () => emit('exit'), class: 'w-7 h-7 flex-shrink-0 rounded-lg bg-slate-50 text-slate-400 hover:bg-slate-100 transition text-xs flex items-center justify-center mr-1' }, '←'),
      h('div', { class: 'text-center flex-1' }, [
        h('p', { class: 'text-xs text-slate-500' }, props.left.label),
        h('p', { class: 'text-lg font-black ' + props.left.color }, String(props.left.value)),
      ]),
      h('div', { class: 'text-center flex-1' }, [
        h('p', { class: 'text-xs text-slate-500' }, props.mid.label),
        h('p', { class: 'text-lg font-black ' + props.mid.color }, String(props.mid.value)),
      ]),
      h('div', { class: 'text-center flex-1' }, [
        h('p', { class: 'text-xs text-slate-500' }, props.right.label),
        h('p', { class: 'text-lg font-black ' + props.right.color }, String(props.right.value)),
      ]),
      h('button', { onClick: () => emit('reset'), class: 'px-3 py-1.5 bg-slate-100 text-slate-600 rounded-xl text-xs font-bold hover:bg-slate-200 active:scale-95 transition' }, [
        h(RotateCcw, { size: 14 }),
      ]),
    ]);
  },
...
/* ---------------- shared setup state ---------------- */
const modes = [
  { id: 'memory', label: "Xotira", icon: Grid3x3, grad: 'from-orange-500 to-orange-600', minPairs: 3, startLabel: "O'yinni boshlash", hint: "Kartalarni oching va juftini toping" },
  { id: 'quiz', label: "Tezkor tanlov", icon: Zap, grad: 'from-indigo-500 to-indigo-600', minPairs: 4, startLabel: "Testni boshlash", hint: "To'g'ri tarjimani 4 tadan birini tanlang" },
  { id: 'type', label: "Yozib toping", icon: PenLine, grad: 'from-green-500 to-green-600', minPairs: 3, startLabel: "Yozishni boshlash", hint: "So'zning tarjimasini o'zingiz yozing" },
  { id: 'battle', label: "Jamoa jangi", icon: Users, grad: 'from-red-500 to-rose-600', minPairs: 0, startLabel: "Jangni boshlash", hint: "Ikki jamoa tuzing, savol-bomba kartalar bilan bellashing" },
] as const;
...
const awardCompletion = async (summary: string, notifTitle: string, notifDesc: string, saveResult?: { pairs: number; attempts: number; time_seconds: number }) => {
...
const resetGame = () => { gameWon.value = false; startGame(); };
const backToSetup = () => {
  clearAllTimers();
  gameWon.value = false;
  screen.value = 'setup';
...
const clearAllTimers = () => { clearInterval(memTimer); clearInterval(typeTimer); };
onUnmounted(() => {
  clearAllTimers();
...
const startMemory = () => {
  ...
  flippedCards.value = []; attempts.value = 0; matchedCount.value = 0; elapsed.value = 0;
    ...
    if (a.pairId === b.pairId && a.type !== b.type) {
      ...
      if (matchedCount.value === validPairs.value.length) {
        ...
        await awardCompletion(
          ...
          { pairs: validPairs.value.length, attempts: attempts.value, time_seconds: elapsed.value }
        );
      }
    } else {
      setTimeout(() => { a.flipped = b.flipped = false; flippedCards.value = []; }, 900);
    }
  }
};
...
const startQuiz = () => {
  ...
  quizIndex.value = 0; quizScore.value = 0; quizStreak.value = 0;
  loadQuizQuestion();
};

const loadQuizQuestion = () => {
  quizAnswered.value = false; quizSelected.value = null; quizPopKey.value++;
  const correct = quizCurrent.value.translation;
  const pool = validPairs.value.filter(p => p.translation !== correct).map(p => p.translation);
...
const answerQuiz = async (opt: string) => {
  ...
  quizAnswered.value = true; quizSelected.value = opt;
  if (opt === quizCurrent.value.translation) {
    quizScore.value++; quizStreak.value++;
  } else {
    quizStreak.value = 0;
  }
...
const quizOptionClass = (opt: string) => {
  if (!quizAnswered.value) return 'bg-white border-slate-200 text-slate-700 hover:border-indigo-300 hover:bg-indigo-50';
  if (opt === quizCurrent.value.translation) return 'bg-green-50 border-green-400 text-green-700 animate-pop';
  if (opt === quizSelected.value) return 'bg-red-50 border-red-300 text-red-500 animate-shake';
...
const typeInputClass = computed(() => {
  if (typeStatus.value === 'correct') return 'border-green-400 bg-green-50 animate-pop';
  if (typeStatus.value === 'wrong') return 'border-red-300 bg-red-50 animate-shake';
...
const startType = () => {
  ...
  typeIndex.value = 0; typeAnswer.value = ""; typeMistakes.value = 0; typeElapsed.value = 0; typeStatus.value = 'idle';
...
const advanceType = async () => {
  if (typeIndex.value + 1 < typeQueue.value.length) {
    typeIndex.value++; typeAnswer.value = ""; typeStatus.value = 'idle';
    nextTick(() => typeInput.value?.focus());
  } else {
    clearInterval(typeTimer);
    await awardCompletion(
...
const submitType = () => {
  ...
  const ok = typeAnswer.value.trim().toLowerCase() === typeCurrent.value.translation.trim().toLowerCase();
...
interface BattleQuestion { question: string; options: string[]; correctIndex: number; }
interface BattleCard { id: number; kind: 'question' | 'bomb'; q?: BattleQuestion; opened: boolean; }

const QUESTIONS_PER_TEAM = 20;
...
const battleTeamSize = computed(() => Math.floor((Math.max(3, battlePlayerCount.value) - 1) / 2));
...
const battleLobbyPlayers = ref<{ id: string; name: string; team: BattleTeam }[]>([]);
const battleChannel = ref<any>(null);
const codeCopied = ref(false);

// shared game state
const battleScreen = ref<'config' | 'lobby' | 'play' | 'over'>('config');
const battleTeams = ref<{ team1: BattleCard[]; team2: BattleCard[] }>({ team1: [], team2: [] });
...
const battleRevealResult = ref<{ correct: boolean; chosen: number; correctIndex: number } | null>(null);
const bombFlashTeam = ref<BattleTeam | null>(null);

const team1Players = computed(() => battleLobbyPlayers.value.filter(p => p.team === 'team1'));
const team2Players = computed(() => battleLobbyPlayers.value.filter(p => p.team === 'team2'));
const activeQuestion = computed(() => {
  if (!battleActiveCard.value) return null;
  return battleTeams.value[battleActiveCard.value.team][battleActiveCard.value.idx]?.q || null;
});
const battleWinnerText = computed(() => {
  if (battleScores.value.team1 === battleScores.value.team2) return "Durrang!";
  return battleScores.value.team1 > battleScores.value.team2 ? "1-guruh g'alaba qozondi!" : "2-guruh g'alaba qozondi!";
});
...
const teamMemberCount = (team: BattleTeam) => battleConnMode.value === 'online'
  ? battleLobbyPlayers.value.filter(player => player.team === team).length
  : battleTeamSize.value;
const activeTeamMemberCount = computed(() => battleActiveCard.value ? teamMemberCount(battleActiveCard.value.team) : 0);
const activeVoteCount = computed(() => battleVotedPlayers.value.size);
const activeVoteProgress = computed(() => activeTeamMemberCount.value ? Math.min(100, (activeVoteCount.value / activeTeamMemberCount.value) * 100) : 0);
const canManuallyResolve = computed(() => !!battleActiveCard.value && !battleRevealResult.value && (battleConnMode.value === 'offline' || battleRole.value === 'host'));
...
const battleCardClass = (card: BattleCard, team: BattleTeam) => {
  const base = team === 'team1' ? 'bg-blue-500 text-white' : 'bg-rose-500 text-white';
...
const battleOptionClass = (i: number) => {
  if (battleRevealResult.value) {
    if (i === battleRevealResult.value.correctIndex) return 'bg-green-50 border-green-400 text-green-700';
    if (i === battleRevealResult.value.chosen) return 'bg-red-50 border-red-300 text-red-500';
...
const buildBattleCards = (questions: BattleQuestion[]): BattleCard[] => {
  ...
  const cards: BattleCard[] = questions.map((q, i) => ({ id: i, kind: 'question', q, opened: false }));
  for (let i = 0; i < bombCount; i++) cards.push({ id: 1000 + i, kind: 'bomb', opened: false });
...
const generateBattleQuestions = async (topic: string) => {
  ...
  try {
    ...
    const mapTeam = (arr: any[] | undefined): BattleQuestion[] => (Array.isArray(arr) ? arr : [])
      .filter(q => q && typeof q.question === "string" && Array.isArray(q.options) && q.options.length === 4)
      .slice(0, QUESTIONS_PER_TEAM)
      .map(q => ({
        question: q.question,
        options: q.options,
        correctIndex: Math.max(0, ['A', 'B', 'C', 'D'].indexOf(String(q.correct).trim().toUpperCase())),
      }));
...
const startBattleOffline = async () => {
  if (!battleTopic.value.trim()) { battleError.value = "Mavzuni kiriting."; return; }
  if (battleTeamSize.value < 1) { battleError.value = "Kamida 3 ta o'yinchi kerak."; return; }
...
const genRoomCode = () => Math.random().toString(36).substring(2, 7).toUpperCase();

const joinBattleChannel = async (code: string) => {
  battleChannel.value = supabase.channel(`battle_${code}`, { config: { broadcast: { self: true } } });
  battleChannel.value.on('broadcast', { event: 'battle' }, ({ payload }: any) => handleBattleEvent(payload));
...
const HOST_ORIGIN_EVENTS = ['card_open', 'reveal', 'clear_active', 'game_start', 'game_over', 'lobby_sync'];
...
const handleBattleEvent = (payload: any) => {
  ...
  if (battleRole.value === 'host' && HOST_ORIGIN_EVENTS.includes(payload.type)) return;

  switch (payload.type) {
    case 'join': {
      if (battleRole.value !== 'host') return;
      const t1 = battleLobbyPlayers.value.filter(p => p.team === 'team1').length;
      const t2 = battleLobbyPlayers.value.filter(p => p.team === 'team2').length;
      const team: BattleTeam = t1 <= t2 ? 'team1' : 'team2';
      battleLobbyPlayers.value.push({ id: payload.id, name: payload.name, team });
      sendBattleEvent({ type: 'lobby_sync', players: battleLobbyPlayers.value });
      break;
    }
    case 'lobby_sync': {
    ...
    case 'vote': {
      ...
      battleVotes.value[payload.optIdx] = (battleVotes.value[payload.optIdx] || 0) + 1;
      if (battleRole.value === 'host') {
        const teamSize = battleLobbyPlayers.value.filter(p => p.team === battleActiveCard.value!.team).length;
        if (teamSize > 0 && battleVotedPlayers.value.size >= teamSize) resolveBattleCard();
      }
      break;
    }
    case 'reveal': {
      battleScores.value = payload.scores;
      battleRevealResult.value = { correct: payload.correct, chosen: payload.chosen, correctIndex: payload.correctIndex };
      break;
    }
    case 'clear_active': {
...
const joinBattleRoom = async () => {
  ...
  battleError.value = "";
  await joinBattleChannel(battleRoomCode.value);
  sendBattleEvent({ type: 'join', id: battleMyId.value, name: battleJoinName.value.trim() });
  screen.value = 'battle';
  battleScreen.value = 'lobby';
};

const startBattleOnline = async () => {
  if (battleLobbyPlayers.value.length < 2) { battleError.value = "Kamida 2 o'yinchi qo'shilishi kerak."; return; }
  const cards = await generateBattleQuestions(battleTopic.value.trim());
  if (!cards) return;
  battleTeams.value = cards;
  battleScores.value = { team1: 0, team2: 0 };
  sendBattleEvent({ type: 'game_start', teams: cards, players: battleLobbyPlayers.value });
  battleScreen.value = 'play';
};
...
const triggerBombFlash = (team: BattleTeam) => {
  ...
  setTimeout(() => { bombFlashTeam.value = null; }, 600);
};
...
const openBattleCard = (team: BattleTeam, idx: number) => {
  ...
  if (battleConnMode.value === 'online' && battleRole.value !== 'host') {
    sendBattleEvent({ type: 'request_open', team, idx, playerId: battleMyId.value });
    return;
  }
  ...
  if (card.kind === 'bomb') {
    ...
    if (battleConnMode.value === 'online') sendBattleEvent({ type: 'card_open', team, idx, kind: 'bomb', scores: { ...battleScores.value } });
    checkBattleEnd();
    return;
  }
  ...
  battleVotedPlayers.value = new Set();
  battleRevealResult.value = null;
  if (battleConnMode.value === 'online') sendBattleEvent({ type: 'card_open', team, idx, kind: 'question' });
  startBattleTimer();
};
...
const resolveBattleCard = () => {
  ...
  battleRevealResult.value = { correct, chosen, correctIndex: card.q.correctIndex };

  if (battleConnMode.value === 'online') {
    sendBattleEvent({ type: 'reveal', scores: { ...battleScores.value }, correct, chosen, correctIndex: card.q.correctIndex });
  }

  setTimeout(() => {
    battleActiveCard.value = null;
    battleRevealResult.value = null;
    if (battleConnMode.value === 'online') sendBattleEvent({ type: 'clear_active' });
    checkBattleEnd();
  }, 1800);
};
...
const checkBattleEndForNonHost = () => {
  if (battleConnMode.value === 'online' && battleRole.value !== 'host' && battleAllOpened()) {
    battleScreen.value = 'over';
  }
};
