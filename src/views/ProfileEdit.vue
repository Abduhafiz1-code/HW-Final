  ...
  <div class="min-h-screen bg-gradient-to-b from-slate-50 to-white dark:from-slate-950 dark:to-slate-900 px-4 pt-6 pb-28">
    <div class="max-w-lg mx-auto">
      <div class="flex items-center gap-3 mb-6">
        ...
        <h1 class="text-xl font-black text-slate-900 dark:text-white">Profilni tahrirlash</h1>
      </div>
      ...
      <!-- Form -->
      <div class="bg-white dark:bg-white/5 rounded-3xl border border-slate-200 dark:border-white/10 p-5 flex flex-col gap-4 mb-4">
        <div>
          <label class="text-xs font-bold text-slate-400 uppercase tracking-wide mb-1.5 block">Ism familiya</label>
        ...
        <div>
          <label class="text-xs font-bold text-slate-400 uppercase tracking-wide mb-1.5 block">Email</label>
          <input v-model="email" type="email" placeholder="email@misol.com"
            class="w-full px-3.5 py-3 rounded-xl border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-slate-900 text-sm font-semibold text-slate-800 dark:text-white focus:outline-none focus:border-orange-400 transition" />
          <p class="text-[11px] text-slate-400 mt-1.5 leading-relaxed">
            Email o'zgartirilsa, yangi manzilga tasdiqlash xati yuboriladi — havolani bosgach amalga oshadi.
          </p>
        </div>

        <div>
          <label class="text-xs font-bold text-slate-400 uppercase tracking-wide mb-1.5 block">Telefon raqam</label>
          <input v-model="phone" type="tel" placeholder="+998 90 123 45 67"
            class="w-full px-3.5 py-3 rounded-xl border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-slate-900 text-sm font-semibold text-slate-800 dark:text-white focus:outline-none focus:border-orange-400 transition" />
        </div>
      </div>

      <p v-if="message" class="text-xs font-semibold mb-3 px-1" :class="ok ? 'text-green-500' : 'text-red-500'">{{ message }}</p>
...
import { ArrowLeft, Camera, Loader } from "@lucide/vue";
...
const save = async () => {
  ...
  saving.value = true;
  try {
    const profileRes = await authStore.updateProfile(cleanName, undefined, phone.value.trim());
    if (!profileRes) {
    ...
    if (cleanEmail && cleanEmail !== authStore.user?.email) {
      ...
      message.value = "Saqlandi! Yangi emailni tasdiqlash uchun pochtangizni tekshiring.";
      ok.value = true;
      return;
    }
