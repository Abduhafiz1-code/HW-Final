<template>
  <div class="flex flex-col h-screen bg-[#F7F9FC] dark:bg-slate-900">
    <div
      ...
      <RouterLink to="/"
        class="w-10 h-10 rounded-xl bg-slate-100 dark:bg-slate-700 flex items-center justify-center text-slate-600 dark:text-slate-300 hover:bg-slate-200 transition">
        ←</RouterLink>
      <div
        class="w-10 h-10 rounded-2xl bg-gradient-to-br from-orange-400 to-orange-600 flex items-center justify-center text-xl">
        🤖</div>
      <div class="flex-1">
        <p class="font-black text-slate-900 dark:text-white text-sm">AI O'qituvchi</p>
        <p class="text-xs font-semibold"
          :class="authStore.isPremium ? 'text-green-500' : authStore.canUseAI ? 'text-orange-500' : 'text-red-500'">
          {{ authStore.isPremium ? '👑 Premium — Cheksiz' : authStore.canUseAI ? `${3 - authStore.aiUsageCount} marta qoldi` : '❌ Limit tugadi' }}
        </p>
      </div>
      <RouterLink v-if="!authStore.isPremium" to="/premium"
        class="px-3 py-1.5 bg-orange-50 text-orange-600 text-xs font-bold rounded-xl hover:bg-orange-100 transition">👑
        Premium</RouterLink>
    ...
    <div ref="msgContainer" class="flex-1 overflow-y-auto px-4 py-4 space-y-4">
      <div v-if="messages.length === 0" class="flex flex-col items-center justify-center py-12 text-center">
        ...
        <p class="text-slate-500 text-sm mt-2 max-w-xs">Matematika, fizika, ingliz tili — istalgan savol!</p>
        <div v-if="!authStore.isPremium"
          ...
          Sizda <strong>{{ 3 - authStore.aiUsageCount }}</strong> ta bepul so'rov qoldi.
    ...
    <div v-if="showLimitModal" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 px-4">
      <div class="bg-white dark:bg-slate-800 rounded-3xl p-8 text-center max-w-sm w-full shadow-2xl">
        ...
        <p class="text-slate-500 text-sm mt-2">3 ta bepul so'rovdan foydalandingiz. Premium oling!</p>
