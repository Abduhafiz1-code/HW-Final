    ...
    <div ref="msgContainer" class="flex-1 overflow-y-auto px-4 py-4 space-y-4" @click="handleContainerClick">
      <div v-if="messages.length === 0" class="flex flex-col items-center justify-center py-12 text-center">
        ...
        <div v-if="!authStore.isPremium"
          ...
          <RouterLink to="/premium" class="mt-1 font-bold underline inline-flex items-center gap-1">Premium olish <ArrowRight :size="13" /></RouterLink>
        </div>
      ...
      <div v-for="msg in messages" :key="msg.id">
        ...
        <div v-else class="flex gap-3">
          <div
            class="w-8 h-8 rounded-xl bg-gradient-to-br from-orange-400 to-orange-600 flex items-center justify-center flex-shrink-0 mt-1">
...
import { Bot, Crown, Lock, Trash2, ArrowLeft, SendHorizontal } from '@lucide/vue';
...
const escapeHtml = (str: string) =>
  str.replace(/&/g, '&' + 'amp;').replace(/</g, '&' + 'lt;').replace(/>/g, '&' + 'gt;');
...
const renderMarkdown = (raw: string): string => {
  ...
  text = text.replace(/```(\w*)\n?([\s\S]*?)```/g, (_match, lang, code) => {
    ...
    const html = `<div class="code-block my-2 rounded-xl overflow-hidden border border-slate-700/40"><div class="flex items-center justify-between bg-slate-800 px-3 py-1.5"><span class="text-[10px] font-bold tracking-wide text-slate-400">${label}</span><button type="button" class="copy-btn flex items-center gap-1 text-[11px] font-semibold text-slate-300 hover:text-white transition" data-code-id="code-${idx}"><span class="copy-label">Copy</span></button></div><pre class="bg-slate-900 text-slate-100 text-xs leading-relaxed p-3 overflow-x-auto m-0"><code id="code-${idx}">${safeCode}</code></pre></div>`;
