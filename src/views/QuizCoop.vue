    ...
    <div class="max-w-lg mx-auto">
      <div class="flex items-center gap-3 mb-6">
        <RouterLink to="/chat"
          ...
          ←</RouterLink>
        <div>
          <h1 class="text-xl font-black text-slate-900">🎯 Birgalikda Test</h1>
        ...
        <RouterLink v-if="!authStore.isPremium" to="/premium"
          class="ml-auto px-3 py-1.5 bg-orange-50 text-orange-600 text-xs font-bold rounded-xl hover:bg-orange-100 transition">
          👑 Premium</RouterLink>
      </div>
      ...
      <div v-if="!authStore.isPremium && !authStore.canUseCoop"
        ...
        <RouterLink to="/premium"
          class="mt-5 inline-block px-6 py-3 bg-orange-500 text-white font-bold rounded-2xl hover:bg-orange-600 transition">
          👑 Premium olish</RouterLink>
      </div>

      <div v-else>
        <!-- Incoming requests -->
        <div v-if="!quizStore.session && quizStore.incomingRequests.length > 0" class="space-y-3 mb-5">
          <p class="text-sm font-bold text-slate-600">📨 Kiruvchi takliflar:</p>
          <div v-for="req in quizStore.incomingRequests" :key="req.id"
            ...
            <button @click="acceptAndStart(req.id)"
              class="px-4 py-2 bg-indigo-600 text-white font-bold rounded-xl text-sm hover:bg-indigo-700 transition">Qabul
              ✓</button>
          </div>
        </div>
        ...
        <div v-if="!quizStore.session" class="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
          <div class="text-center py-10 px-8 border-b border-slate-100">
            ...
            <p class="text-slate-500 text-sm mt-2 px-4">Chat dan do'stingga test taklifini yuboring. 4 kishigacha
              birgalikda!</p>
            <RouterLink to="/chat"
              class="mt-6 inline-block px-6 py-3 bg-orange-500 text-white font-bold rounded-2xl hover:bg-orange-600 transition">
              Chat →</RouterLink>
            <button @click="refreshData"
              class="block mx-auto mt-3 text-xs text-indigo-500 font-semibold hover:text-indigo-700">↺
              Yangilash</button>
          </div>
        ...
        <div v-else-if="quizStore.session.status === 'waiting'"
          ...
          <button @click="refreshSession" class="mt-4 text-xs text-indigo-500 font-semibold">↺ Yangilash</button>
        </div>
        ...
        <div v-else-if="quizStore.session.status === 'active'">
          ...
          <div v-if="currentQ" class="bg-white rounded-3xl border border-slate-200 p-6 shadow-sm">
            ...
            <div v-if="quizStore.myAnswer" class="mt-4 text-center">
              ...
              <button @click="nextQ"
                class="px-6 py-2.5 bg-orange-500 text-white font-bold rounded-2xl hover:bg-orange-600 transition">Keyingi
                →</button>
            </div>
          </div>
        </div>
        ...
        <div v-else-if="quizStore.session.status === 'finished'"
          ...
          <div class="text-6xl mb-4">{{ myScore > partnerScore ? '🏆' : myScore === partnerScore ? '🤝' : '💪' }}</div>
          ...
          <button @click="quizStore.session = null; refreshData()"
...
const nextQ = () => quizStore.nextQuestion();
</script>
