    ...
    <div class="w-full max-w-md">
      ...
      <div class="bg-white rounded-3xl shadow-xl border border-slate-100 p-8">
        ...
        <div v-if="tab === 'login'" class="space-y-4">
          ...
          <p v-if="authStore.error" class="text-red-500 text-sm bg-red-50 rounded-xl px-4 py-2">{{ authStore.error }}
          </p>
          <button @click="handleLogin" :disabled="authStore.loading"
            class="w-full py-3 rounded-2xl bg-orange-500 hover:bg-orange-600 text-white font-black text-base shadow-lg transition active:scale-95 disabled:opacity-60">
        ...
        <div v-else class="space-y-4">
          ...
          <div v-if="selectedRole === 'teacher'">
            ...
            <input v-model="teacherCode" type="password" placeholder="Maxsus kod kiriting"
              class="w-full px-4 py-3 rounded-2xl border border-slate-200 bg-slate-50 text-slate-900 focus:outline-none focus:border-indigo-400 transition" />
            <p class="text-xs text-slate-400 mt-1">* Kod maktab ma'muriyatidan olinadi</p>
          </div>
          <p v-if="authStore.error" class="text-red-500 text-sm bg-red-50 rounded-xl px-4 py-2">{{ authStore.error }}
          </p>
          <p v-if="signupSuccess" class="text-green-600 text-sm bg-green-50 rounded-xl px-4 py-2">✅ Muvaffaqiyatli
            ro'yxatdan o'tdingiz!</p>
          <button @click="handleSignup" :disabled="authStore.loading"
            class="w-full py-3 rounded-2xl bg-indigo-600 hover:bg-indigo-700 text-white font-black text-base shadow-lg transition active:scale-95 disabled:opacity-60">
